  ...
  Y.requestModules(['memory']).then(function () {
    ...
    class OperationStore extends Y.AbstractDatabase {
      constructor (y, options) {
        ...
        var osPath = path.join(dbpath, 'os');
        var dsPath = path.join(dbpath, 'ds');
        var ssPath = path.join(dbpath, 'ss');

        const mkdirP = path => new Promise((res, rej) => {
        ...
        this.ready = Promise.all([
          ...
          this.os = level(osPath, levelOptions)
          this.ds = level(dsPath, levelOptions)
          this.ss = level(ssPath, levelOptions)
        })
      }
